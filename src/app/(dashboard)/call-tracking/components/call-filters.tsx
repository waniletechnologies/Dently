			...
			<CardContent className="p-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
					<div>
						...
						<Select defaultValue="apr21-may5">
							...
							<SelectContent>
								{callTrackingData.filterOptions.dateRanges.map((range: any) => (
					...
					<div>
						...
						<Select defaultValue="not-booked">
							...
							<SelectContent>
								{callTrackingData.filterOptions.bookingStatuses.map(
									(status) => (
					...
					<div>
						...
						<Select defaultValue="pending">
							...
							<SelectContent>
								{callTrackingData.filterOptions.followUpStatuses.map(
									(status) => (
										<SelectItem key={status.value} value={status.value}>
											{status.label}
										</SelectItem>
				...
				<div className="flex justify-end gap-2">
					<Button
						variant="outline"
						onClick={handleReset}
						className="flex cursor-pointer bg-[#F4F2F2] items-center gap-2"
					>
						<X className="h-4 w-4" />
						Reset
					</Button>
