...
export function PatientsTable({ onViewPatient, onEditPatient, onDeletePatient }) {
    ...
    <Table>
      <TableHeader>
        <TableRow className="border-b border-[#e1e1e1]">
          ...
          <TableHead className="text-[#848484] font-medium">Last Visit</TableHead>
          <TableHead className="text-[#848484] font-medium">Status</TableHead>
          <TableHead className="text-[#848484] font-medium text-right">Action</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {patients.map((patient) => (
          <TableRow key={patient.id} className="border-b border-[#f0f0f0] hover:bg-[#f8f9fa]">
            <TableCell className="font-medium text-[#171717]">
              <Link href={`/client-management/patients/${patient.id}`} className="hover:text-[#ffa048]">
                {patient.name}
              </Link>
            </TableCell>
            <TableCell className="text-[#171717]">{patient.phone}</TableCell>
            <TableCell className="text-[#171717]">{patient.email}</TableCell>
            <TableCell className="text-[#171717]">{patient.lastVisit}</TableCell>
            ...
            <TableCell className="text-right">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-[160px]">
                  <DropdownMenuItem onClick={() => onViewPatient(patient.id)} className="flex items-center gap-2">
                    <Eye className="h-4 w-4" />
                    View
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onEditPatient(patient.id)} className="flex items-center gap-2">
