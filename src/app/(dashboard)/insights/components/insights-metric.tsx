		...
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{InsightsData.metrics.map((metric) => {
				return (
					<Card
						key={metric.title}
						className="bg-white h-full p-0 border-[#e1e1e1]"
					>
						<CardContent className="p-5">
							<div className="flex w-full h-20 flex-col justify-between">
								<div className="flex items-center  w-full justify-between">
									<p className="font-normal text-[13.26px] leading-[18.94px] tracking-[0] text-[#000000]">
										{metric.title}
									</p>
								</div>
								<div className="flex items-center gap-3">
									...
									<div className="flex items-center gap-1">
										{metric.change.startsWith('+') ? (
											...
											<FiTrendingDown className="text-[#D43A3A]" />
										)}
										<span
